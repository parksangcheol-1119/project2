...
*/
select
    max(salary) MaxPay, min(salary) as MinPay, round(avg(salary)) AvgPay
from employees;
...
*/
select
    job_id, trim(to_char(max(salary),'990,000')) MaxPay, 
...
*/
select
    job_id, count(*) ClerkCount
from employees
group by job_id 
--order by count(*) desc;--정렬방법1
order by ClerkCount desc;--정렬방법2
/*
...
group by job_id
order by count(*) desc;
--급여는 물리적으로 존재하는 컬럼이므로 where절에 조건을 걸어야한다.

/*
5. 급여최고액과 최저액의 차액을 출력하시오. 
*/
select
    max(salary) - min(salary) "차액"
from employees;

/*
6. 직급별 사원의 최저급여를 출력하시오. 관리자를 알수없는 
사원 및 최저급여가 3000미만인 그룹은 제외시키고 결과를 급여의 
내림차순으로 정렬하여 출력하시오.
*/
select  
    job_id, min(salary)
...
select
    ...
    count(*) "사원수", 
    to_char(avg(salary),'$999,000') "급여평균"
from employees
group by department_id;
