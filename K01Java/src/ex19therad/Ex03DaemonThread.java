...
class YesThread extends Thread 
{
	...
	public YesThread() {}
	/*
	Thread 클래스의 생성자를 호출할때 전달하는 매개변수를 통해
	해당 쓰레드의 이름을 지정할 수 있다.. 지정된 이름은 getName()를 통해
	반환할 수 있다.
	 */
...
/*
 종속쓰레드(Daemon thread)
 	...
 	주로 프로그램의 보저역할로 배경음악, 로그자동저장 등의 업무를
...
class DaemonThread extends Thread {
		...
		@Override
		public void run() {
			while(true) {//무한루프 (스레드가 죽을때 같이 죽으니까)
				System.out.println(
						String.format("[쓰레드명:%s]jass가 흘러요~", getName()));
...
public class Ex03DaemonThread {
	...
	//	yt1.setName("첫번째스레드"); //메소드를 통해 쓰레드의 이름을 부여
		yt1.start();
		
		YesThread yt2 = new YesThread();//이름이 지정되지 않으면 자동으로 부여된다.
		yt2.setPriority(Thread.MAX_PRIORITY);//우선순위를 맥스니까 10으로 지정
		yt2.start();
