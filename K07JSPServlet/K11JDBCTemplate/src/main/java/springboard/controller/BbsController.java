...
public class BbsController {
	...
	/*
	@Autowired
		스프링 설정 파일에서 생성된 빈을 자동으로 주입받고 싶을 때
		사용한다. 타입을 기반으로 자동 주입된다. 만약 해당 타입의 빈이
		존재하지 않으면 에러가 발생되고 서버를 시작할 수 없다.
			- 생성자, 멤버 변수, 메소드 (setter)에 적용 가능하다.
			- 타입을 이용해 자동으로 프로퍼티 값을 설정한다.
			- 해당 어노테이션은 멤버 변수에만 적용할 수 있다. 
			- 함수 내의 지역 변수에는 사용할 수 없다.
			- 타입을 통해 자동으로 설정되므로 같은 타입이 2개이상 존재하면 예외가 발생한다.
	 */
	...
	/*
	멤버 변수로 선언하여 클래스에서 전역적으로 사용할 수 있다.
	해당 클래스의 모든 command 객체는 BbsCommandImpl 인터페이스를 구현하여
	정의한다.
	 */
	...
	public String list(Model model, HttpServletRequest req) {
		...
		/*
		사용자로부터 받은 모든 요청은 request 객체에 저장된다. 
		requset 객체를 ListCommand 객체로 전달하기 위해 model
		객체에 저장한 후 매개변수로 전달한다.
		 */
	...
	public String writeAction(Model model, SpringBbsDTO springBbsDTO) {/*
		글쓰기 페이지에서 전송된 모든 폼값을 SpringBbsDTO 객체가 한번에 받는다.
		Spring에서는 커맨드 객체를 통해 이와같이 폼값을 편리하게 받을 수 있다.
		 */
