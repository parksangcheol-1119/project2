...
//	}
	...
	public String uploadAction(Model model, MultipartHttpServletRequest req) {
		...
		//맵을 저장한 후 View로 전달하기 위한 리스트 컬렉션
		...
		try {
			...
			while(itr.hasNext()) {
				...
				fileMap.put("originalName", originalName);//원본파일명
				fileMap.put("saveFileName", saveFileName);//저장된파일명
				fileMap.put("title", title);//제목
				
				//위 정보를 저장한 Map을 ArrayList에 저장한다. 
		...
		return "06FileUpload/uploadAction2";
	}
